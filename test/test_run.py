...
def main():
    ...
    if training_args.do_predict:
        ...
        if is_regression:
            eval_result = compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids))
        elif is_multi_label:
            eval_result = compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids))
        else:
            eval_result = compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids))
        ...
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                ...
                for index, (prediction, label) in enumerate(zip(preds, label_ids)):
                    if is_regression:
                        writer.write(f"{index}\t{prediction:3.3f}\t{label:3.3f}\n")
                    elif is_multi_label:
                        # Recover from multi-hot encoding
                        prediction = [label_list[i] for i in range(len(prediction)) if prediction[i] == 1]
                        label = [label_list[i] for i in range(len(label)) if label[i] == 1]
